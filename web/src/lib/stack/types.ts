...
// Default stack used internally by geometry generators for via resistance computation
const DEFAULT_LAYERS: StackLayer[] = [
	...
	{ id: 'm2', name: 'Metal 2', type: 'metal', z: 302.0, thickness: 0.5, rsh: 0.03, color: '#d9513c', gdsLayers: ['crossings', 'windings_m2'], visible: true },
	{ id: 'via23', name: 'Via 2-3', type: 'via', z: 302.5, thickness: 0.6, color: '#6e6e78', gdsLayers: ['vias', 'vias1'], visible: true },
	{ id: 'm3', name: 'Metal 3', type: 'metal', z: 303.1, thickness: 1.2, rsh: 0.01, color: '#e8944a', gdsLayers: ['windings'], visible: true },
];

/** Default stack for internal geometry computations (via resistance) */
export function createDefaultStack(): ProcessStack {
	return {
		name: 'Default',
		layers: DEFAULT_LAYERS.map(l => ({ ...l, gdsLayers: [...l.gdsLayers] })),
