import type { Polygon, LayerMap, Port, GeometryResult, SymmetricInductorParams } from './types';
...
export function buildSymmetricInductor(params: SymmetricInductorParams): GeometryResult {
	const { Dout, width, spacing, center_tap } = params;
	const ar = params.aspectRatio ?? 1;

	// Polygons are the single source of truth.
	const layers = generateLegacyPolygons(params);

	// Ports sit on the outermost winding near the bottom edge.
	const ps = params.portSpacing ?? spacing;
	const portXOffset = center_tap ? ps + width : (ps + width) / 2;
	const portY = -Dout / 2 - width;
	const shiftY = makeAspectShiftY(Dout, ar);

	const ports: Port[] = [
		{ name: 'P1', x: -portXOffset, y: shiftY(portY), layer: 'windings', role: 'signal' },
		{ name: 'P2', x: portXOffset, y: shiftY(portY), layer: 'windings', role: 'signal' },
	];
	if (center_tap) {
		// For N<=2 the CT metal is the winding stub itself; only N>2 routes it
		// out on the dedicated centertap layer (matches generateLegacyPolygons).
		const ctLayer = params.N <= 2 ? 'windings' : 'centertap';
		ports.push({ name: 'CT', x: 0, y: shiftY(portY), layer: ctLayer, role: 'centertap' });
	}

	// Apply aspect ratio — stretch the straight sides only (y=0 fixed).
	if (ar !== 1) {
		for (const key of Object.keys(layers) as (keyof LayerMap)[]) {
			const polys = layers[key];
			if (polys) layers[key] = polys.map(p => mapY(p, shiftY));
		}
	}
