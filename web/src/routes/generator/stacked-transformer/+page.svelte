	...
	{#snippet sidebar()}
		<ParamSidebar onexport={doExport}>
			<div class="param-section"><h4>Geometry</h4>
				...
				<div class="f"><span>Aspect</span><div class="fi"><button onclick={() => set('aspectRatio', Math.max(0.1, (p.aspectRatio ?? 1) - 0.1))}>-</button><input type="number" value={p.aspectRatio ?? ''} placeholder="1.0" step="0.1" oninput={e => { const v = parseInput(e); set('aspectRatio', v && v > 0 ? v : undefined); }}/><button onclick={() => set('aspectRatio', (p.aspectRatio ?? 1) + 0.1)}>+</button><em></em></div></div>
