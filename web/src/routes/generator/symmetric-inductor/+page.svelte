<script lang="ts">
	...
	function doExportJson() {
		...
		const json = exportForFEM(layers, result.network, stack, {
			generator: 'symmetric_inductor',
			params: { ...p },
		});
		downloadFemJson(json, 'symmetric_inductor.fem.json');
	}
	...
	{#snippet sidebar()}
		<ParamSidebar onexport={doExport} onexportjson={doExportJson}>
			<div class="param-section"><h4>Geometry</h4>
				...
				<ParamField label="Width" value={p.width} unit="um" step={0.5} min={0.1} onchange={v => set('width', v ?? 16)} />
